<%@ page language="java" pageEncoding="UTF-8" %>
<link rel="stylesheet" type="text/css" href="../css/iconfont.css"/>
<header id="header" class="head-animated">
    <div class="container">
        <div class="text-logo pull-left">
            <a href="/" title="Dreamwings">木东驿站</a>
            <p>在这里重新踏上旅途……</p>
        </div>
        <div id="navigation-wrapper" class="pull-right">
            <ul id="menu-main" class="menu">
                <li id="menu-item-1857"
                    class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1855 ">
                    <a href="guangchang.jsp"><i class="iconfont">&#xe662;</i> 广场</a>
                </li>
                <li id="menu-item-2065" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1855">
                    <a href="zhuye.jsp"><i class="iconfont">&#xe612;</i> 主页</a>
                </li>
                <li id="menu-item-2346"
                    class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1855">
                    <a href="/readerwall"><i class="iconfont">&#xe699;</i> 日志</a>
                </li>
                <li id="menu-item-3906"
                    class="menu-item menu-item-type-custom menu-item-object-custom current-menu-item current_page_item menu-item-1857">
                    <a href="myfriend_fb.jsp"><i class="iconfont">&#xe865;</i>好友</a>
                </li>
                <li id="menu-item-2356"
                    class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1855">
                    <a href="friend.jsp"><i class="iconfont">&#xe613;</i> 友人</a></li>
                <li id="menu-item-1855" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1855">
                    <a href="information.jsp"><i class="iconfont">&#xe615;</i> 个人信息</a></li>
                <li id="menu-item-2066" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-2066">
                    <a href="guanyu.jsp"><i class="iconfont">&#xe611;</i>关于</a></li>
                <li id="menu-item-2067" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-2066">
                    <a href="../exit"><i class="iconfont">&#xe600;</i> 退出驿站</a></li>
            </ul>
        </div>
        <div class="menu-mobile"></div>
    </div>
</header>